...
{
    public class LoyaltyProgramXUnitTest
    {
        [Fact]
        ...
        public void ExampleFromRequirements_GoldTierMixedCart()
        {
            ...
            Assert.Equal(50.0, receipt.GetTotalPrice()); // Verify total is €50
            Assert.Equal(70.0, points, 2);
        }
        ...
        {
            // ASSERT
            Assert.Equal(0, LoyaltyProgram.GetTierThreshold(LoyaltyTier.Bronze));
            Assert.Equal(500, LoyaltyProgram.GetTierThreshold(LoyaltyTier.Silver));
            Assert.Equal(1000, LoyaltyProgram.GetTierThreshold(LoyaltyTier.Gold));
        }
    }
}